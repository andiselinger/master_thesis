...
\label{sec:seq_mpi}
The just discussed scalable and non-scalable implementations first perform the calculation of one row of $C$ with the local part of $B$ ($A_{\textit{loc, diag~}} B_{\textit{loc, diag}}$), and subsequently with the non-local part of $B$ ($A_{\textit{loc, off~}} B_{\textit{non, loc}}$), before continuing to the next row of $C$, where the calculation with the local part is started again. This means that the non-local part of $B$ is needed very early in the computation, so the program has to wait until this non-local part is made available to the process. 
...
\end{equation}
where only the last product relies on non-local rows. When calculating these products, the multiplications 
...
\caption{The three arrays with the column indices of the results of the multiplications $A_{\textit{loc,diag~}}B_{\textit{loc,diag}}$, $A_{\textit{loc,diag~}} B_{\textit{loc,off}}$ and $A_{\textit{loc,off}}$ have to be merged.}
\label{fig:rowmerge}
\end{figure}

Ideally, all non-local rows have arrived by now and the computation of $A_{\textit{loc, off}} B_{\textit{nonloc}}$ can start without any delays (L26). This computation is also executed by an existing, sequential version of matrix multiplication. 
