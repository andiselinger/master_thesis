...
\end{align}
Of course, averaging the error makes it smooth very quickly, but that does not mean that it becomes small. This effect can be seen in the example in Fig.~\ref{fig:mg_error_smoothing}.

\begin{figure}[h]
	\centering
	\includegraphics[width=1.\textwidth]{chapters/chapter02/mg_error_smoothing}
	\caption{The Gauss-Seidel iteration smooths the error considerably after only very few iterations. However, even though it becomes smooth quickly, the error does not become much smaller.}
	\label{fig:mg_error_smoothing}
\end{figure}
